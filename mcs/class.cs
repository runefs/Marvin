...
namespace Mono.CSharp
{
	...
	public abstract class TypeContainer : DeclSpace, ITypeDefinition, ITypesContainer
	{
		...
		private void RemoveFromMemberList(MemberCore mc, List<MemberCore> alist){
			ordered_member_list.Remove (mc);
			if(methods != null){
		       alist.Remove (mc);
			}
		}
		...
		protected virtual void DefineContainerMembers (System.Collections.IList mcal) // IList<MemberCore>
		{
			if (mcal != null) {
				for (int i = 0; i < mcal.Count; ++i) {
					...
					try{
					    var method = mc as Method;
                        //if (method == null || (method.ModFlags & Modifiers.NOEMIT) == 0){
                            mc.Define();
                        //}
					} catch (Exception e) {

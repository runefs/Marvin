...
namespace Mono.CSharp
{
	public class TypeSpec : MemberSpec
	{
        ...
        private readonly Dictionary<int, Interface> _typesAndContracts = new Dictionary<int, Interface>();
        

        private bool CheckContracts(TypeSpec[] arguments)
        {
            for (var i = 0; i < arguments.Length; ++i)
            {
                var arg = arguments[i];
                if(_typesAndContracts.ContainsKey(i)){
                    if (!arg.FullfillsContract(_typesAndContracts[i]))
                    {
                        return false;
                    }
                }
        ...
        public void AddDeferedChecks(string parameterName, Predicate<TypeSpec> check)
        {
            TypeManager.AddDeferedChecks(ToString(), parameterName, check);
        }
        
        public bool CheckAllDefered(IDictionary<string,TypeSpec> targs){
            return TypeManager.CheckAllDefered(ToString(), targs);
        }

        public bool CheckContracts()
        {
            return CheckContracts(TypeArguments);
        }

        public void AddTypesAndContracts(int index, Interface contracts)
        {
            _typesAndContracts.Add(index, contracts);
        }

...
namespace Mono.CSharp
{
    ...
    public static class MemberSpecExtensions
    {
        public static bool IsContractDummyMethod(this MemberSpec self)
        {
            return self != null && self.IsStatic && self.DeclaringType.Name.StartsWith("><_extensionContainerFor");
        }
    }
	//
	...
	public class Invocation : ExpressionStatement
	{
        ...
        private bool IgnoreRoleMethodLookup { get; set; }
		protected override Expression DoResolve (ResolveContext ec)
		{
			Expression member_expr = null;
			var atn = expr as ATypeNameExpression;
            if (atn != null)
            {
                member_expr = atn.LookupNameExpression
                    (ec, MemberLookupRestrictions.InvocableOnly | MemberLookupRestrictions.ReadAccess);
                if (member_expr == null && IgnoreRoleMethodLookup)
                    return null; //Tried to resolve as role method and failed
                if (!IgnoreRoleMethodLookup)
                {
                    if (!IgnoreRoleMethodLookup)
                    {
                        // Try to resolve as a RoleMethod
                        var roleExpression = expr as MemberAccess;

                        if (roleExpression != null)
                        {
                            var fld = roleExpression.LeftExpression as FieldExpr;
                            var selfRef = roleExpression.LeftExpression as ParameterReference;
                            var self = selfRef != null ? selfRef.Parameter as Self : null;
                            var rolePlayer = fld ?? (Expression)selfRef;
                            if ((fld != null && fld.IsRole) || (self != null))
                            {
                                var temp = member_expr;
                                string fieldName,contractName;
                                if (fld != null)
                                {
                                    fieldName = fld.Name;
                                    contractName = CSharpParser.GetCurrentRoleContractName(fld.DeclaringType.ToString(), fieldName);
                                }
                                else
                                {
                                    fieldName = "self";
                                    contractName = self.RoleContractName;
                                }
                                var contract = CSharpParser.RoleContracts.ContainsKey(contractName) ? CSharpParser.RoleContracts[contractName] : null;
                                var roleMethodAccess = contract.MethodExistInContract(roleExpression.Name, arguments, ec)
                                               ? roleExpression
                                               : new MemberAccess(new This(loc), roleExpression.Name, loc, true);
                                bool nullArguments = false;
                                if (arguments == null)
                                {
                                    arguments = new Arguments(1);
                                    nullArguments = true;
                                }
                                arguments.Insert(0, new Argument(rolePlayer));
                                ec.Report.DisableReporting();
                                var roleMethodInvocation =
                                    new Invocation(roleMethodAccess, arguments) { IgnoreRoleMethodLookup = true }.DoResolve(ec);
                                ec.Report.EnableReporting();
                                if (roleMethodInvocation != null)
                                {
                                    return roleMethodInvocation;
                                }
                                //Not a role method, revert to the starting point and do regular look up
                                member_expr = temp;
                                if (nullArguments)
                                {
                                    arguments = null;
                                ...
                                {
                                    var existInContract = contract.MethodExistInContract(roleExpression.Name, arguments, ec);
                                    if (!existInContract)
                                    {
                                        ec.Report.Error
                                            (1061, loc,
                                             "Role contract `{0}' does not contain a definition for `{1}'. It should either be declared as part of the contract or not used",
                                             fieldName, roleExpression.Name);
	...
	public class New : ExpressionStatement, IMemoryLocation
	{
		...
		protected override Expression DoResolve (ResolveContext ec)
		{
			...
			if (type == null)
				return null;
            if(!type.CheckContracts()){
                ec.Report.Error (10311, loc, "Cannot use type `{0}' for this role",
					TypeManager.CSharpName (type));
            }
			eclass = ExprClass.Value;
			if (type.IsPointer) {
	...
	public class MemberAccess : ATypeNameExpression
	{
        ...
        public MemberAccess(Expression expr, string identifier, Location loc)
            : this(expr, identifier,loc,false)
        {
        }
		public MemberAccess (Expression expr, string identifier, Location loc, bool ignoreErrors)
			: base (identifier, loc)
		{
			this.expr = expr;
		...
		public override Expression LookupNameExpression (ResolveContext rc, MemberLookupRestrictions restrictions)
		{
            ...
            if (field != null && field.IsRole)
            {
                member_lookup = MemberLookup(rc, errorMode, field.DeclaringType, Name, lookup_arity, restrictions, loc);
            }
            else
            {
                //ensure definite assignment of member_lookup
                member_lookup = null;
            }
			if (expr_type.BuiltinType == BuiltinTypeSpec.Type.Dynamic && member_lookup == null) {
				...
				if (member_lookup == null) {
					...
					if (MethodGroupExpr.IsExtensionMethodArgument (expr)) {
						...
						if (methods != null) {
							...
							var res = emg.Resolve (rc);
                            if (emg.Candidates.All(c => c.IsContractDummyMethod())) { 
                            }
                            return res;
						}
					}

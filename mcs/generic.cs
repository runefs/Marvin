...
namespace Mono.CSharp {
	...
	public class TypeParameterSpec : TypeSpec
	{
        ...
        public TypeSpec RolePlayerContract { get; set; }
...
//			this.state = openType.state;
	...
	struct ConstraintChecker
	{
		...
		public bool CheckAll (MemberSpec context, TypeSpec[] targs, TypeParameterSpec[] tparams, Location loc)
		{
            ...
            var ts = context as TypeSpec;
            if (ts == null) return true;

            var args = targs.Select((t, i) => new { Spec = t, Key = tparams[i].Name }).ToDictionary(x => x.Key, x => x.Spec);
            return ts.CheckAllDefered(args);
		}

        static TypeSpec GetGenericTypeParameter(TypeSpec t)
        {
            TypeSpec arg = null;
            if(t.IsGenericParameter){
                arg = t;
            } else if(t.TypeArguments != null && t.TypeArguments.Length > 0){
                //only support one at the moment
                var list = t.TypeArguments.Select(a => GetGenericTypeParameter(a)).Where(a => a != null);
                arg = t.TypeArguments.Where(a => a.IsGenericParameter).Concat(list).Distinct().SingleOrDefault();
            }
            return arg;
        }
		...
		bool CheckConstraint (MemberSpec context, TypeSpec atype, TypeParameterSpec tparam, Location loc)
		{
            ...
            if (tparam.RolePlayerContract != null) {
                var genericParameter = GetGenericTypeParameter(atype);
                if (genericParameter != null)
                {
                    var self = this;
                    mc.CurrentType.AddDeferedChecks(genericParameter.Name, (t) => {
                        return self.CheckContractAgainstTypeArgument(t, tparam.RolePlayerContract, ref loc);
                    });
                }
                else if(!CheckContractAgainstTypeArgument(atype, tparam.RolePlayerContract, ref loc))
                {
                    return false;
                }
        ...
        private IEnumerable<MethodBase> GetMethods(TypeSpec t)
        {
            while (t != null)
            {
                foreach(var m in t.GetMetaInfo().__GetDeclaredMethods()){
                    yield return m;
                }
                t = t.BaseType;
            }
        ...
        public bool CheckContractAgainstTypeArgument(TypeSpec atype, TypeSpec contract, ref Location loc)
        {
            var methodLookup = GetMethods(atype).ToLookup(m => m.Name);
            
            foreach (var method in contract.GetMetaInfo().__GetDeclaredMethods())
            {
                var methodSig = method.Name + "(" + string.Join(",", method.GetParameters().Select(p => p.ParameterType.Name)) + ")";
                if (!methodLookup.Contains(method.Name))
                {
                    if (mc != null)
                    {
                        mc.Module.Compiler.Report.Error(10311, loc,
                            "The type `{0}` did not meet the RolePlayerContract. Needed method missing `{1}`",
                            TypeManager.CSharpName(atype), methodSig);
                    }
                    return false;
                }
                var methods = methodLookup[method.Name];
                var methodSignature = method.MethodSignature;
                foreach (var candidate in methods)
                {
                    var candidateSignature = candidate.MethodSignature;
                    if (!(candidateSignature.MatchParameterTypes(methodSignature.ParameterTypes) &&
                        candidateSignature.GetReturnType(null) == methodSignature.GetReturnType(null)))
                    {
                        if (mc != null)
                        {
                            mc.Module.Compiler.Report.Error(10312, loc,
                                "The type `{0}` did not meet the RolePlayerContract. Needed method does not match signature `{1}´",
                                TypeManager.CSharpName(atype), methodSig);
                        }
                        return false;
                    }
	...
	{
        private static readonly Dictionary<string,Dictionary<string, List<Predicate<TypeSpec>>>> _deferedChecks = new Dictionary<string,Dictionary<string, List<Predicate<TypeSpec>>>>();

        public static void AddDeferedChecks(string typeName,string parameterName, Predicate<TypeSpec> check)
        {
            if (!_deferedChecks.ContainsKey(typeName))
            {
                _deferedChecks.Add(typeName, new Dictionary<string, List<Predicate<TypeSpec>>>());
            }
            var dic = _deferedChecks[typeName];
            if (!dic.ContainsKey(parameterName))
            {
                dic.Add(parameterName, new List<Predicate<TypeSpec>>());
            }
            dic[parameterName].Add(check);
        }

        public static bool CheckAllDefered(string typeName, IDictionary<string, TypeSpec> specs)
        {
            if (_deferedChecks.Count == 0) return true;
            if (!_deferedChecks.ContainsKey(typeName)) return true;
            var dic = _deferedChecks[typeName];
            
            foreach (var key in dic.Keys)
            {
                var predicates = dic[key];

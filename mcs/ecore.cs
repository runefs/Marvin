...
namespace Mono.CSharp {
	...
	public abstract class MemberExpr : Expression
	{
		...
		public bool ResolveInstanceExpression (ResolveContext rc, Expression rhs)
		{
			...
			if (InstanceExpression == null || InstanceExpression is TypeExpr) {
                ...
                Contract.Assert(InstanceExpression != null);
				return false;
			}
		...
		public virtual MemberExpr ResolveMemberAccess (ResolveContext ec, Expression left, SimpleName original)
		{
            ...
            Contract.Requires(left != null || (left == null && original == null));
	...
	public class MethodGroupExpr : MemberExpr, OverloadResolver.IBaseMembersProvider
	{
		...
		public virtual MethodGroupExpr OverloadResolve (ResolveContext ec, ref Arguments args, OverloadResolver.IErrorHandler cerrors, OverloadResolver.Restrictions restr)
		{
		    ...
		    var field = InstanceExpression as FieldExpr;
            ...
            if (best_candidate.IsRoleMethod)
            {
                InstanceExpression = new This(loc);
            }
		...
		public virtual MethodGroupExpr LookupExtensionMethod (ResolveContext rc)
		{
            ...
            if (emg.best_candidate.IsContractDummyMethod())
            {
                InstanceExpression = temp;
                return null;
            }
        ...
        //
        // Role mthod lookup prior to all over look ups
        //
        public virtual MethodGroupExpr LookupRoleMethod(ResolveContext rc, ref Arguments ars, OverloadResolver r, OverloadResolver.Restrictions restr)
        {
            if (InstanceExpression == null)
                return null;
            var tempInstance = InstanceExpression;
            var tempArgs = ars;
            InstanceExpression = InstanceExpression.Resolve(rc);
            if (!IsExtensionMethodArgument(InstanceExpression))
                return null;
            ...
            // TODO: When in probing mode do IsApplicable only and when called again do VerifyArguments for full error reporting
            best_candidate = r.ResolveMember<MethodSpec>(rc, ref tempArgs);
            if (best_candidate == null)
                goto notFound;
            ...
            if (best_candidate.Kind == MemberKind.Method && (restr & OverloadResolver.Restrictions.ProbingOnly) == 0)
            {
                if (InstanceExpression != null)
                {
                    if (best_candidate.IsExtensionMethod && tempArgs[0].Expr == InstanceExpression)
                    {
                        InstanceExpression = null;
                    }
                    else
                    {
                        if (best_candidate.IsStatic && simple_name != null)
                        {
                            InstanceExpression = ProbeIdenticalTypeName(rc, InstanceExpression, simple_name);
                        }

                        InstanceExpression.Resolve(rc);
                    }
                }
            ...
            var base_override = CandidateToBaseOverride(rc, best_candidate);
            if (base_override == best_candidate)
            {
                best_candidate_return = r.BestCandidateReturnType;
            }
            ...
            return this;
        notFound:
            InstanceExpression = tempInstance;
            return null;
        }
	...
	public struct OverloadResolver
	{
        ...
        public bool LookingUpRoleMethod { get; set; }
				...
				try {
					...
					do {
						for (int i = 0; i < type_members.Count; ++i) {
                            ...
                            var candidate_rate = IsApplicable
                                        (rc, ref candidate_args, args_count, ref member, pm, ref params_expanded_form,
                                         ref dynamic_argument, ref rt);
		...
		{
            if (rc.Report.IsDisabled) return;
	...
	public class FieldExpr : MemberExpr, IDynamicAssign, IMemoryLocation, IVariableReference
	{
	    ...
	    private readonly bool _useSpecIsRole;
        ...
        public FieldExpr(FieldSpec spec, Location loc) : this(spec,loc,true) {}

	    public FieldExpr(FieldSpec spec, Location loc, bool useSpecIsRole)
		{
			this.spec = spec;
	        _useSpecIsRole = useSpecIsRole;
	        this.loc = loc;
        ...
        public bool IsRole { get { return spec.IsRole && _useSpecIsRole; } }
